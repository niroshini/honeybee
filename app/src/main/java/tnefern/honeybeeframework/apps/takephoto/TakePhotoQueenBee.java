...
public class TakePhotoQueenBee extends QueenBee {
	private SearchImage imageSearch = null;

	public TakePhotoQueenBee(Activity pAct) {
		super(pAct);
		...
		if (pParam != null) {
			if (pParam instanceof Job) {
				...
				String extension = FileFactory.getInstance().getFileExtension(
						fileName);
				if ((extension
						.equalsIgnoreCase(TakePhotoConstants.FILE_EXTENSION_JPEG))
						|| (extension
					...
					TakePhotoResult.getInstance().addToMap(cj.stringValue,
							cj.intValue);
					JobPool.getInstance().incrementDoneJobCount();
					TakePhotoResult.getInstance().incrementDeleDoneJobs();
	...
	public ResultFactory getResultFactory() {
		return TakePhotoResult.getInstance();
	}
