	...
	public void onCreate(Bundle savedInstanceState) {
		...
		exitBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Intent deleIntent = new Intent(FinishedTakePhotoDelegatorActivity.this, HoneybeeCrowdActivity.class);
				deleIntent.setFlags( Intent.FLAG_ACTIVITY_CLEAR_TOP );
				startActivityForResult(deleIntent, TakePhotoConstants.FINISHED_DELEGATOR);
		...
		stats.setText("Speedup = "+ TakePhotoResult.getInstance().getSpeedup()+" Stats: "+ TakePhotoResult.getInstance().getCompareString());
		loadFirstImage();
	...
	private void iterateOnce(){
		...
		if(iter.hasNext()){
			...
			if(f!=null){
				...
				int num = TakePhotoResult.getInstance().getNumberOfFaces(s);
				numberFound.setText("Number of faces in "+s+" = "+num);
			}
		}else{
			if( !TakePhotoResult.getInstance().getFilesInFolder().isEmpty()){
				iter = TakePhotoResult.getInstance().getFilesInFolder().iterator();
			}
		}
	}
